 ...
 *  Dialogue box for selection of ghost (pending) item colour
  ...
  IdBlock *const id_block, void *const handle)
{
  ...
  int colour_block[2] = {(int)EditWin_get_ghost_colour(edit_win), 0};
  ...
  IdBlock *const id_block, void *const handle)
{
  ...
  EditWin_set_ghost_colour(edit_win, cdcse->colour_block[0]);
...
void GhostCol_created(ObjectId const id)
{
  GhostCol_id = id;
  ...
  for (size_t i = 0; i < ARRAY_SIZE(handlers); ++i)
  {
    EF(event_register_toolbox_handler(id, handlers[i].event_code,
                                      handlers[i].handler, &GhostCol_id));
  }
}

void GhostCol_show(EditWin const *const edit_win)
{
  EditWin_show_dbox(edit_win, Toolbox_ShowObject_AsMenu, GhostCol_id);
}
