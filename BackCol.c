  ...
  IdBlock *const id_block, void *const handle)
{
  ...
  int colour_block[2] = {(int)EditWin_get_bg_colour(edit_win), 0};
  ...
  IdBlock *const id_block, void *const handle)
{
  ...
  EditWin_set_bg_colour(edit_win, cdcse->colour_block[0]);
...
void BackCol_created(ObjectId const id)
{
  BackCol_id = id;
  ...
  for (size_t i = 0; i < ARRAY_SIZE(handlers); ++i)
  {
    EF(event_register_toolbox_handler(id, handlers[i].event_code,
                                      handlers[i].handler, &BackCol_id));
  }
}

void BackCol_show(EditWin const *const edit_win)
{
  EditWin_show_dbox(edit_win, Toolbox_ShowObject_AsMenu, BackCol_id);
}
